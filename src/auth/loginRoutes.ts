...
export function registerLoginRoutes(app: Express) {
  app.get("/auth/metabase/login", (_req, res) => {
    const { metabaseLoginUrl, metabaseSessionTtlHours } = getAuthConfig();
    res.type("html").send(renderLoginPage({ metabaseLoginUrl, metabaseSessionTtlHours }));
  });
  ...
  app.post("/auth/metabase/login", async (req, res) => {
    ...
    if (!username || !password) {
      res.status(400).type("html").send(renderMessage("登录失败", "请输入 Metabase 账号和密码。"));
      return;
    }
...
function renderLoginPage(input: { metabaseLoginUrl: string; metabaseSessionTtlHours: number }) {
  return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Metabase 授权</title>
  <style>
...
<body>
  <h1>授权 app-data MCP 访问 Metabase</h1>
  <p>请输入 <code>${escapeHtml(input.metabaseLoginUrl)}</code> 的 Metabase 账号密码。MCP 会换取用户 session，用于后续按你的 Metabase 权限只读查询数据。</p>
  ...
  <p class="hint">不会把密码写入本地文件；仅保存 Metabase session 和个人 MCP token 的哈希。默认有效期 ${input.metabaseSessionTtlHours} 小时。</p>
</body>
</html>`;
}
...
<body>
  <h1>授权成功</h1>
  <p>已为 ${escapeHtml(input.user)} 保存 Metabase session，有效期至 ${escapeHtml(input.expiresAt)}。请把下面的个人 MCP token 配到你的 AI 助手中；token 只展示这一次。</p>
  <div class="token-row">
