...
import { isManagedToolAvailable } from "./toolStore.js";
...
export async function auditToolCall<T>(
  ...
  try {
    if (!await isManagedToolAvailable(toolName, getRequestContext().user)) {
...
async function createAuditTableUnderLock(client: PoolClient) {
  ...
  await client.query(`alter table ${tableName} add column if not exists ai_client_version text`);
  await client.query(`alter table ${tableName} add column if not exists question_text text`);
