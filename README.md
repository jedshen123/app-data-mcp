...
- `metabase:card:*` / `metabase:model:*`: 调用 Metabase card query endpoint；Model 在 Metabase API 中同样通过 card query 执行。
...
Metabase 同步会为 dashboard/card/model 写入权限快照 `asset.access`，包括 collection、creator、archived、personal collection、同步时间等。MCP 使用两级过滤：

卡片与 Model 同步会在读取 `/api/card` 列表后，以受限并发继续读取 `/api/card/:id` 详情，并优先使用详情中的 `dataset_query`、`result_metadata`、参数和更新时间。Model 会保存为 `metabase:model:<id>` 和 `type=model`；字段元信息区分 `name`、`displayName` 与真实 `description`。从普通 Card 转换为 Model 时，会迁移原记录并保留开放状态和后台人工配置。详情请求失败时会保留列表数据，并在同步日志和资产 `warnings` 中标记。
